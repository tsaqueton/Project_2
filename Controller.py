...
class Controller(QMainWindow, Ui_MainWindow):
        ...
        if self.textEdit_Descripton.toPlainText() == '':
            ...
            if num == 0:
                if self.listWidget.count() == 0:
                    num = 1
                    self.to_do_dict.update({num: self.textEdit_Descripton.toPlainText()})
                    self.update_list()
                else:
    ...
    def save_list(self):
        with open('to_do.csv', 'w') as csvfile:
            for key in self.to_do_dict.keys():
                csvfile.write("%s,%s\n" % (key, self.to_do_dict[key]))
        ...
        try:
            with open("to_do.csv", "r") as file_dict:
                ...
                for lines in csvFile:
                    key = lines[0]
                    value = lines[1]
                    self.to_do_dict.update({int(key): value})
            self.update_list()
        except FileNotFoundError:
            pass


        """self.textEdit_Descripton.keyPressEvent = self.keyPressEvent
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key_Return:
            ...
            self.textEdit_Descripton.append()"""